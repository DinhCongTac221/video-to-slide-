...
def detect_unique_screenshots(video_path, output_folder_screenshot_path):
    ...
    for frame_count, frame_time, frame in get_frames(video_path):
        ...
        # Check if motion has stopped and capture the frame
        ...
        # Check if the scene is still in motion or if we're still in warm-up mode
        elif captured and (p_diff >= MAX_PERCENT or frame_count <= WARMUP):
            captured = False
...
def initialize_output_folder(video_path):
    ...
    output_folder_screenshot_path = os.path.join(OUTPUT_SLIDES_DIR, base_filename)
